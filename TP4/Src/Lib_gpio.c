...
void LibGPIO_INT_Init(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, LibGPIO_Mode_t GPIO_Mode,
    LibGPIO_OType_t GPIO_OType, LibGPIO_PuPd_t GPIO_PuPd, LibGPIO_Speed_t GPIO_Speed);


void Lib_delay(long d)
	{
	long t;
...
void LibGPIO_Conf(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, LibGPIO_Mode_t GPIO_Mode,
    LibGPIO_OType_t GPIO_Type, LibGPIO_Speed_t GPIO_Speed)
    {
...
void GPIO_Writelow(GPIO_TypeDef* GPIOx, uint8_t LowVal)
    {
    uint16_t out= GPIO_ReadOutputData(GPIOx);

    out &= 0xFF00;
    out |= LowVal;
    GPIO_Write(GPIOx, out);
    }


void GPIO_WriteHigh(GPIO_TypeDef* GPIOx, uint8_t HighVal)
    {
    uint16_t out= GPIO_ReadOutputData(GPIOx);
...
void LibGPIO_Init(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, LibGPIO_Mode_t GPIO_Mode,
    LibGPIO_OType_t GPIO_OType, LibGPIO_PuPd_t GPIO_PuPd, LibGPIO_Speed_t GPIO_Speed)
    {
	/* Check input */
	if (GPIO_Pin == 0x00)
...
void LibGPIO_InitAlternate(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, LibGPIO_OType_t GPIO_OType,
    LibGPIO_PuPd_t GPIO_PuPd, LibGPIO_Speed_t GPIO_Speed, uint8_t Alternate) {
	uint32_t pinpos;
