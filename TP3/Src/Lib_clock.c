...
/**
 * variables globales pour gestion des alarmes
 * à remettre à 0 après gestion de l'alarme
 */
uint8_t LibClock_AlarmA= 0;
uint8_t LibClock_AlarmB= 0;
...
/**
  * @brief  Check if current alarm time is reached
  * @param  RTC_Alarm = RTC_Alarm_A or RTC_Alarm_B
  * @retval 0 or 1
  */
uint8_t LibClock_OnAlarm(uint32_t RTC_Alarm)
	{
	if (RTC_Alarm == RTC_Alarm_A)
		return LibClock_AlarmA;
	if (RTC_Alarm == RTC_Alarm_B)
		return LibClock_AlarmB;
...
// RTC Alarms interrupt request handler
